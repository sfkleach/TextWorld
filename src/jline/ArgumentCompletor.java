...
public class ArgumentCompletor implements Completor {
	...
	public int complete( final String buffer, final int cursor, final List candidates ) {
		...
		//	This is the place where -null- can be passed as a buffer parameter.
		// 	I don't see any advantage in that, so I'm fixing it here.  (Steve Leach)
		final String c_arg = list.getCursorArgument();
		final int ret = comp.complete( c_arg == null ? "" : c_arg, argpos, candidates );
		if ( ret == -1 )  return -1;


		//	Steve Leach: I think the original code is off-by-one.  There is
		//	a compensating error in CandidateListCompletionHandler.
